        ...
        private void dfs(int[] nums, int n, List<List<Integer>> lists, ArrayList<Integer> list,boolean[] visit) {
            if (n == nums.length) {
                ...
                for (int i = 0; i < nums.length; i++) {
                    ...
                    // 所以当vis[i - 1] == 1时（回溯前进dfs）说明nums[i - 1]和nums[i]分别属于两层递归中，也就是我们要用这两个数分别放在数组的两个位置，这时不需要去重。
                    // 但是当vis[i - 1] == 0时（回溯完）说明nums[i - 1]和nums[i]属于同一层递归中（只是for循环进入下一层循环），也就是我们要用这两个数放在数组中的同一个位置上，这就是我们要去重的情况。
