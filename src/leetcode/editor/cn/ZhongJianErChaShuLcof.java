...
class ZhongJianErChaShuLcof {
    ...
    class Solution {
        ...
        public TreeNode dfs(int[] preorder, int[] inorder, int index) {
            if (preorder.length == 0 || inorder.length == 0) {
                return null;
            }
            if (index == preorder.length) {   //根节点在先序遍历中的位置
                return new TreeNode(-1);
            }
            TreeNode root = new TreeNode(preorder[index]);
            int x = 0; //根节点在中序遍历中的位置
            ...
            int[] rightInorder = new int[inorder.length - x - 1];
            for (int i = 0; i < rightInorder.length; i++) {
                rightInorder[i] = inorder[i + x + 1];
            }
            if (leftInorder.length != 0) {
                root.left = dfs(preorder, leftInorder, index + 1);
            }
            if (rightInorder.length != 0) {
                root.right = dfs(preorder, rightInorder, index + leftInorder.length + 1);
            }
            return root;
        }
        ...
        // 返回这棵树的根节点
        private TreeNode buildTree(int[] preorder, HashMap<Integer, Integer> inOrderMap, int preStart, int inStart, int size) {
